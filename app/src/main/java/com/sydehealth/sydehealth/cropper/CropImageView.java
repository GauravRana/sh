package com.sydehealth.sydehealth.cropper;
...
import com.sydehealth.sydehealth.cropper.cropwindow.edge.Edge;
import com.sydehealth.sydehealth.cropper.util.AspectRatioUtil;
import com.sydehealth.sydehealth.cropper.util.HandleUtil;
import com.sydehealth.sydehealth.cropper.util.PaintUtil;
import com.sydehealth.sydehealth.R;
import com.sydehealth.sydehealth.cropper.cropwindow.handle.Handle;
...
public class CropImageView extends ImageView {
    ...
    private RectF mOriginBitMapRect = new RectF();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        Log.d(TAG, "onLayout: ＸＸＸＸＸＸＸＸ");

        mBitmapRect = getBitmapRect();
        mOriginBitMapRect = mBitmapRect;
    ...
    public void initCropWindow() {
        ...
        if (mFixAspectRatio) {
            ...
            // Initialize the crop window with the proper aspect ratio.
            initCropWindowWithFixedAspectRatio(mOriginBitMapRect);

        } else {

            // Initialize crop window to have 10% padding w/ respect to Drawable's bounds.
            final float horizontalPadding = 0.1f * mOriginBitMapRect.width();
            final float verticalPadding = 0.1f * mOriginBitMapRect.height();

            Edge.LEFT.setCoordinate(mOriginBitMapRect.left + horizontalPadding);
            Edge.TOP.setCoordinate(mOriginBitMapRect.top + verticalPadding);
            Edge.RIGHT.setCoordinate(mOriginBitMapRect.right - horizontalPadding);
            Edge.BOTTOM.setCoordinate(mOriginBitMapRect.bottom - verticalPadding);
            invalidate();
        }
    }
