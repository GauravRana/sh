...
public class DrawingView extends View {
    ...
    static public LinkedList<DrawPath> savePath;
    DrawPath mLastDrawPath;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //MainActivity.colorOption.setVisibility(VISIBLE);
        if (!mDrawMode) {
            return false;
        }
        float x;
    ...
    public void undo() {
        ...
        if (savePath != null && savePath.size() > 0) {
            ...
            if(savePath.size() == 1){
                //mPaint.setColor(getResources().getColor(R.color.screeshot_red));
                //MainActivity.colorOption.setVisibility(INVISIBLE);
               // mLastDrawPath = new DrawPath(mPath, mPaint.getColor(), mPaint.getStrokeWidth());
                mLastDrawPath=null;
                savePath.remove(mLastDrawPath);
                invalidate();
            }
    ...
    public void undoAll() {
        Log.d(TAG, "undo: recall all drawing delete");
        if (savePath != null && savePath.size() > 0) {
            // 清空画布
            int sizeOfPath = savePath.size();
